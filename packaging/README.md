...
| File | Purpose |
| --- | --- |
| `build-client.ps1` | Publish WinUI client (win-x64) + zip; optionally compile Inno installer (**v6**, UTF-8 BOM; dumps XamlCompiler diagnostics on failure) |
...
Failed publishes do not create a zip. On failure, **v6** prints filtered XamlCompiler/MSBuild errors and writes `packaging\out\logs\publish-last.log`. Prefer **Developer PowerShell for VS**. If MSB4062 / Pri.Tasks.dll appears, install **Windows App Packaging** + the WinUI workload in Visual Studio Installer.
