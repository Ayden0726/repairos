...
<#
.SYNOPSIS
  Definitive WorkshopOS client 1.2.4 rebuild: kill process, wipe ALL local data,
...
.NOTES
  ...
  You MUST see Connect / pairing UI and version banner Client 1.2.4.
...
$NewRun = Join-Path $env:USERPROFILE 'Desktop\WorkshopOS-Client-1.2.4'
...
Write-Host '=== WorkshopOS Client 1.2.4 - wipe / verify / build / launch ===' -ForegroundColor Cyan
...
# 4) Verify ServerConnect is ALWAYS the initial page + version 1.2.4 in source
...
$checks = [ordered]@{
  ...
  'ServerConnect banner Client 1.2.4' = ($xaml -match 'Client 1\.2\.4')
  'csproj Version 1.2.4' = ($proj -match '<Version>1\.2\.4</Version>')
  'build-client.ps1 default 1.2.4' = ($bld -match 'Version\s*=\s*"1\.2\.4"')
...
# 5) Build 1.2.4 (with or without installer - SkipInstaller by default; remove switch for Inno)
Set-Location $Repo
Write-Host 'Building client 1.2.4 (script v5)...' -ForegroundColor Cyan
...
# powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\build-client.ps1 -Configuration Release -Version 1.2.4
...
# 6) Extract to Desktop\WorkshopOS-Client-1.2.4 and launch THAT exe only
...
Write-Host ''
Write-Host 'You must see Connect / pairing UI and version 1.2.4' -ForegroundColor Green
Write-Host '  - Green banner: Client 1.2.4'
