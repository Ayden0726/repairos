...
<#
.SYNOPSIS
  Definitive WorkshopOS client 1.2.7 rebuild: kill process, wipe ALL local data,
...
.NOTES
  ...
  You MUST see Connect / pairing UI and version banner Client 1.2.7.
...
$NewRun = Join-Path $env:USERPROFILE 'Desktop\WorkshopOS-Client-1.2.7'
...
Write-Host '=== WorkshopOS Client 1.2.7 - wipe / verify / build / launch ===' -ForegroundColor Cyan
...
# 4) Verify ServerConnect is ALWAYS the initial page + version 1.2.7 in source
...
$checks = [ordered]@{
  ...
  'ServerConnect banner Client 1.2.7' = ($xaml -match 'Client 1\.2\.7')
  'csproj Version 1.2.7' = ($proj -match '<Version>1\.2\.7</Version>')
  'build-client.ps1 default 1.2.7' = ($bld -match 'Version\s*=\s*"1\.2\.7"')
...
# 5) Build 1.2.7 (with or without installer - SkipInstaller by default; remove switch for Inno)
Set-Location $Repo
Write-Host 'Building client 1.2.7 (script v5)...' -ForegroundColor Cyan
powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\build-client.ps1 -Configuration Release -Version 1.2.7 -SkipInstaller
# For installer instead: omit -SkipInstaller (needs Inno Setup 6)
# powershell -NoProfile -ExecutionPolicy Bypass -File .\packaging\build-client.ps1 -Configuration Release -Version 1.2.7

$built = Join-Path $Repo 'packaging\dist\WorkshopOS-Client-win-x64-v1.2.7.zip'
if (-not (Test-Path $built)) { throw "Missing $built - build failed or still produced an older version zip." }

# 6) Extract to Desktop\WorkshopOS-Client-1.2.7 and launch THAT exe only
...
Write-Host ''
Write-Host 'You must see Connect / pairing UI and version 1.2.7' -ForegroundColor Green
Write-Host '  - Green banner: Client 1.2.7'
