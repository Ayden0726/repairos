...
<#
.SYNOPSIS
  Definitive WorkshopOS client 1.2.8 rebuild: kill process, wipe ALL local data,
...
.NOTES
  ...
  You MUST see Connect / pairing UI and version banner Client 1.2.8.
...
$NewRun = Join-Path $env:USERPROFILE 'Desktop\WorkshopOS-Client-1.2.8'
...
Write-Host '=== WorkshopOS Client 1.2.8 - wipe / verify / build / launch ===' -ForegroundColor Cyan
...
# 4) Verify ServerConnect is ALWAYS the initial page + version 1.2.8 in source
...
$built = Join-Path $Repo 'packaging\dist\WorkshopOS-Client-win-x64-v1.2.8.zip'
if (-not (Test-Path $built)) { throw "Missing $built - build failed or still produced an older version zip." }

# 6) Extract to Desktop\WorkshopOS-Client-1.2.8 and launch THAT exe only
...
Write-Host ''
Write-Host 'You must see Connect / pairing UI and version 1.2.8' -ForegroundColor Green
Write-Host '  - Green banner: Client 1.2.8'
